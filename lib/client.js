window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function shortcutTargetIsInteractive(target) {
      return target instanceof Element && Boolean(target.closest('button, a[href], input, textarea, select, summary, audio[controls], video[controls], [contenteditable]:not([contenteditable="false"]), [role="button"], [role="textbox"], [role="combobox"], [tabindex]:not([tabindex="-1"])'))
    }
    ...
    const HOST_ARIA = {
      ...
      collapseSidebar: ["收起侧边栏", "Collapse sidebar", "关闭侧边栏", "Close sidebar"],
    }
...
.dsh-mobile-frame.dsh-mobile-sidebar-compat [data-dsh-bux-details-toggle]{top:calc(52px + var(--dsh-mobile-session-row-height,33.8px) + var(--dsh-mobile-session-row-height,33.8px) + env(safe-area-inset-top))!important;z-index:55!important}
...
body.dsh-mobile-active.dsh-mobile-sidebar-compat [data-dsh-bux-details-toggle]{top:calc(52px + var(--dsh-mobile-session-row-height,33.8px) + var(--dsh-mobile-session-row-height,33.8px) + env(safe-area-inset-top))!important;z-index:55!important}
...
body.dsh-mobile-active.dsh-mobile-flat.dsh-mobile-sidebar-compat [data-dsh-bux-details-toggle]{top:calc(52px + var(--dsh-mobile-session-row-height,33.8px) + env(safe-area-inset-top))!important}
...
[data-dsh-bux-settings-panel]{position:relative!important;width:calc(100vw - 24px)!important;max-width:none!important;height:calc(100vh - 24px)!important;max-height:none!important;flex-direction:column!important}
...
`
 ...
 const rowDrag = (row) => fiberPropsFor(row, (props) => props.drag && typeof props.drag.start === "function" && typeof props.drag.end === "function")?.drag || null
      ...
      attachFrameObserver()
      if (!watchingFrame && document.body) bootObserver.observe(document.body, { childList: true, subtree: true })
    ...
    const EMPTY_SUMMARY = { overall: "", recent: "", seq: -1, overallSeq: -1, revision: 0, status: "idle", error: "", usage: null }
    ...
    function startSummaryArchiveCleanup(ctx) {
      ...
      clearArchived()
      return ctx.workspaces.list.subscribe(clearArchived)
    }
    ...
    function ConversationSummary({ sessionId, useSession, summaryDirectory }) {
      ...
      const refresh = React.useCallback(async (force = false) => {
        ...
        const run = async () => {
          ...
          if (cached.revision > startedRevision) {
    ...
    function startConversationSummary(ctx) {
      const onShortcut = (event) => {
        if (event.defaultPrevented || event.repeat || event.isComposing || shortcutTargetIsInteractive(event.target)) return
