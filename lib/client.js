...
`
 ...
 const rowDrag = (row) => fiberPropsFor(row, (props) => props.drag && typeof props.drag.start === "function" && typeof props.drag.end === "function")?.drag || null
      ...
      const titleOfRow = (row) => textOf(row.querySelector(".YDXeBa_title")) || textOf(row).replace(/\s+\d+\s*(?:分钟|小时|天|个月|年|min(?:ute)?s?|h(?:our)?s?|d(?:ay)?s?|mo(?:nth)?s?|y(?:ear)?s?)$/i, "").trim()
        ...
        const groups = nativeGroups.map((group, index) => {
          ...
          const sessions = [...group.querySelectorAll(".YDXeBa_sessionRow")].map((row) => {
            ...
            return {
              ...
              title: titleOfRow(row) || tt("fallback.session"),
              time: timeOfRow(row),
          ...
          return {
            key,
            title: textOf(project) || tt("fallback.workspace"),
            source: group,
            project,
            projectProps,
      ...
      const syncActionIcons = () => {
        if (!shell) return
        const sources = [
          ["new-session", ".hHd-Xa_newSession", "action.newSession"],
          ["new-workspace", SEL_ADD_WORKSPACE, "action.addWorkspace"],
          ["search", ".qDHVXG_searchButton", "action.search"],
          ["view", SEL_VIEW_OPTIONS, "action.viewOptions"],
          ["settings", ".VOzbGW_trigger", "action.settings"],
        ]
      ...
      const buildShell = () => {
        ...
        actions.append(
          makeAction("new-session", tt("action.newSession"), ".hHd-Xa_newSession", "+"),
          makeAction("new-workspace", tt("action.addWorkspace"), SEL_ADD_WORKSPACE, "+"),
          makeAction("search", tt("action.search"), ".qDHVXG_searchButton", "⌕"),
          makeAction("view", tt("action.viewOptions"), SEL_VIEW_OPTIONS, "☷"),
          makeAction("settings", tt("action.settings"), ".VOzbGW_trigger", "⚙"),
        )
    ...
    function startModelPicker(ctx) {
      ...
      const render = () => {
        if (!root) return
        root.querySelector(".mpo-panel")?.setAttribute("aria-label", tt("picker.title"))
        const title = root.querySelector(".mpo-title")
        if (title) title.textContent = tt("picker.title")
        const searchInput = root.querySelector(".mpo-search")
        if (searchInput) searchInput.placeholder = tt("picker.searchPlaceholder")
        root.querySelector(".mpo-close")?.setAttribute("aria-label", tt("picker.close"))
        const effortLabel = root.querySelector(".mpo-foot-label")
        if (effortLabel) effortLabel.textContent = tt("picker.effortLabel")
        ...
        providers.innerHTML = [
          `<button type="button" class="mpo-chip" data-provider="all" data-on="${provider === "all" ? "1" : "0"}">${escapeHtml(tt("picker.all"))}</button>`,
        ...
        root.querySelector(".mpo-grid").innerHTML = visible.length
          ? visible.flatMap((group) => group.models.map((model) => {
            ...
            const vision = model.visionProvider
              ? `<span class="mpo-card-vision" data-provider="${escapeHtml(model.visionProvider)}" data-model="${escapeHtml(model.id)}" data-on="${visionOn ? "1" : "0"}" role="button" aria-label="${escapeHtml(tt("picker.vision"))}"></span>`
              : ""
          ...
          : `<div class="mpo-empty">${escapeHtml(tt(state.status === "loading" ? "picker.loading" : "picker.noMatch"))}</div>`
        ...
        if (!selectedModel || efforts.length === 0) {
          effortsEl.innerHTML = `<button type="button" class="mpo-effort" disabled>${escapeHtml(tt("picker.noEffort"))}</button>`
        } else {
      ...
      const open = () => {
        ...
        root.innerHTML = `
          ...
          <div class="mpo-panel" role="dialog" aria-label="${escapeHtml(tt("picker.title"))}">
            <div class="mpo-head">
              <h2 class="mpo-title">${escapeHtml(tt("picker.title"))}</h2>
              <input class="mpo-search" type="search" placeholder="${escapeHtml(tt("picker.searchPlaceholder"))}">
              <button type="button" class="mpo-close" aria-label="${escapeHtml(tt("picker.close"))}">×</button>
            </div>
            <div class="mpo-providers-wrap"><div class="mpo-providers"></div></div>
            <div class="mpo-grid"></div>
            <div class="mpo-foot">
              <div class="mpo-foot-label">${escapeHtml(tt("picker.effortLabel"))}</div>
      ...
      window.addEventListener(CHANGE, onChange)
      window.addEventListener(LOCALE_CHANGE, render)
      ...
      return () => {
        ...
        window.removeEventListener(CHANGE, onChange)
        window.removeEventListener(LOCALE_CHANGE, render)
    ...
    function startSettingsIcon() {
      ...
      const mark = () => {
        ...
        for (const button of document.querySelectorAll(".VOzbGW_navCell")) {
          ...
          if (label === tt("section.label")) button.dataset.dshBuxNav = "1"
      ...
      observer.observe(document.body, { childList: true, subtree: true, attributes: true, characterData: true, attributeFilter: ["class"] })
      mark()
      return () => {
        observer.disconnect()
    ...
    exports.apply = apply
    exports.inject = ["slots", "sessions", "modelDirectories", "locale"]
