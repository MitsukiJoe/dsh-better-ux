window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    let markedSessionHeader = null
    let markedDetailsToggle = null
    let markedDetailsPanel = null
    let markedSettingsLayer = null
    let markedSettingsCells = []
    ...
    function markHostSemantics() {
      ...
      if (markedSessionHeader && markedSessionHeader !== header) delete markedSessionHeader.dataset.dshBuxSessionHeader
      markedSessionHeader = header || null
      ...
      for (const marked of [markedDetailsPanel, markedDetailsToggle]) {
        if (!marked) continue
      ...
      const detailsToggleHost = sidePanelToggle?.parentElement || null
      markedDetailsPanel = sidePanelHost || null
      markedDetailsToggle = detailsToggleHost
      if (sidePanelHost) sidePanelHost.dataset.dshBuxDetailsPanel = "1"
      if (detailsToggleHost) detailsToggleHost.dataset.dshBuxDetailsToggle = "1"
      const section = slotNode("settings.section")
      const panel = section?.closest('[role="dialog"]')
      const settingsLayer = panel?.parentElement || null
      if (markedSettingsLayer && markedSettingsLayer !== settingsLayer) delete markedSettingsLayer.dataset.dshBuxSettingsLayer
      markedSettingsLayer = settingsLayer
      let settingsCells = []
      if (panel) {
      ...
      for (const previous of markedSettingsCells) {
        if (settingsCells.includes(previous)) continue
        delete previous.dataset.dshBuxSettingsCell
        delete previous.dataset.dshBuxNav
      }
      markedSettingsCells = settingsCells
      return { frame, sidebar, center, details }
    }

    function reconcileListRow(scroll, cache, entries, hooks) {
      const seen = new Set()
      const nodes = []
      for (const entry of entries) {
        const key = hooks.keyOf(entry)
        const signature = hooks.signatureOf(entry)
        seen.add(key)
        let record = cache.get(key)
        if (!record) {
          record = { node: null, signature: null, entry }
          cache.set(key, record)
        }
        record.entry = entry
        if (!record.node || record.signature !== signature) {
          record.node?.remove()
          const node = hooks.createNode(entry)
          record.node = node
          record.signature = signature
          hooks.bindNode(node, record)
        }
        nodes.push(record.node)
      }
      const wanted = new Set(nodes)
      let cursor = scroll.firstChild
      for (const node of nodes) {
        ...
        else scroll.insertBefore(node, cursor)
      }
      while (cursor) {
        const next = cursor.nextSibling
        cursor.remove()
...
.dsh-mobile-frame.dsh-mobile-sidebar-compat [data-dsh-bux-details-toggle]{top:calc(48px + var(--dsh-mobile-session-row-height,33.8px) + var(--dsh-mobile-session-row-height,33.8px) + env(safe-area-inset-top) + 4px)!important;z-index:55!important}
...
body.dsh-mobile-active.dsh-mobile-sidebar-compat [data-dsh-bux-details-toggle]{top:calc(48px + var(--dsh-mobile-session-row-height,33.8px) + var(--dsh-mobile-session-row-height,33.8px) + env(safe-area-inset-top) + 4px)!important;z-index:55!important}
...
body.dsh-mobile-active.dsh-mobile-flat.dsh-mobile-sidebar-compat [data-dsh-bux-details-toggle]{top:calc(48px + var(--dsh-mobile-session-row-height,33.8px) + env(safe-area-inset-top) + 4px)!important}
...
`
 ...
 const rowDrag = (row) => fiberPropsFor(row, (props) => props.drag && typeof props.drag.start === "function" && typeof props.drag.end === "function")?.drag || null
      ...
      const bindLongPress = (item, record, kind) => {
        let startX = 0
        let startY = 0
        let active = false
        item.addEventListener("pointerdown", (event) => {
          ...
          pressTimer = window.setTimeout(() => {
            ...
            active = false
            openSortCapsule(item, record.entry, kind)
          }, 420)
        })
      ...
      const workspaceSignature = (entry) => `w:${entry.key}:${entry.title}:${entry.key === selectedGroupKey ? 1 : 0}:${entry.project ? 1 : 0}`
      const sessionSignature = (entry) => `s:${entry.id}:${entry.title}:${entry.time}:${entry.selected ? 1 : 0}:${entry.pendingInteraction || ""}:${entry.running ? 1 : 0}:${entry.runningSubagentCount}:${entry.completed ? 1 : 0}:${entry.status ? 1 : 0}`
      const workspaceHooks = {
        keyOf: (entry) => entry.key,
        signatureOf: workspaceSignature,
        createNode: (entry) => makeItem(entry, "workspace"),
        bindNode: (node, record) => bindLongPress(node, record, "workspace"),
      }
      const sessionHooks = {
        keyOf: (entry) => entry.id,
        signatureOf: sessionSignature,
        createNode: (entry) => makeItem(entry, "session"),
        bindNode: (node, record) => bindLongPress(node, record, "session"),
      }
      const render = (refreshChrome = true) => {
        if (!shell) return
        ...
        if (!flat) {
          for (const entry of data.groups) sigParts.push(workspaceSignature(entry))
        }
        ...
        for (const entry of sessionEntries) sigParts.push(sessionSignature(entry))
        ...
        if (lastListLang !== LOCALE.lang) {
          lastListLang = LOCALE.lang
          clearListCaches()
          workspaceScroll.replaceChildren()
          sessionScroll.replaceChildren()
        }
        reconcileListRow(workspaceScroll, workspaceItems, flat ? [] : data.groups, workspaceHooks)
        reconcileListRow(sessionScroll, sessionItems, sessionEntries, sessionHooks)
      ...
      const scheduleSource = () => {
        if (!mobileActive() || sourceRaf !== null) return
        sourceRaf = window.requestAnimationFrame(() => {
          sourceRaf = null
          if (!shell || !watchedSource?.isConnected) {
            schedule()
            return
          }
          render(false)
      ...
      const observeSource = () => {
        ...
        if (column) sourceObserver?.observe(column, { childList: true, subtree: true, characterData: true, attributes: true, attributeFilter: ["class", "aria-selected", "aria-expanded", "aria-label"] })
      }
      ...
      sourceObserver = new MutationObserver(() => scheduleSource())
      attachFrameObserver()
      ...
      const stopSessions = ctx.sessions.list.subscribe(() => {
        if (!watchedSource?.isConnected) schedule()
      })
      ...
      return () => {
        ...
        if (sourceRaf !== null) window.cancelAnimationFrame(sourceRaf)
    ...
    const summaryArchiveEpochs = new Map()
    const summaryArchiveEpoch = (sessionId) => summaryArchiveEpochs.get(String(sessionId || "")) || 0
    const advanceSummaryArchiveEpoch = (sessionId) => {
      const key = String(sessionId || "")
      const next = summaryArchiveEpoch(key) + 1
      summaryArchiveEpochs.set(key, next)
      return next
    }
      ...
      try {
        while (stillArchived()) {
          if (baseRevision === null) {
            const stateRes = await fetch(STATE_ROUTE + "?sessionId=" + encodeURIComponent(sessionId), { signal })
            ...
            if (!stillArchived()) return true
            baseRevision = state.summary
              ? state.summary.revision
              : Number.isSafeInteger(state.summaryRevision) && state.summaryRevision >= 0
                ? state.summaryRevision
                : 0
          }
          if (!stillArchived()) return true
          const res = await fetch(STATE_ROUTE + "?sessionId=" + encodeURIComponent(sessionId) + "&baseRevision=" + baseRevision, { method: "DELETE", signal })
          if (!stillArchived()) return true
    ...
    function startSummaryArchiveCleanup(ctx) {
      ...
      const cancelEntry = (entry) => {
        ...
        entry.token.controller.abort()
        if (summaryDeleteTokens.get(entry.sessionId) === entry.token) {
          advanceSummaryArchiveEpoch(entry.sessionId)
          summaryDeleteTokens.delete(entry.sessionId)
        }
      }
      ...
      const createEntry = (sessionId, fresh) => {
        ...
        const token = { owner, controller: new AbortController(), retryTimer: null, retryDelay: 1000, running: null, complete: false }
        const entry = { sessionId: key, token, fresh, knownRevision: null, localCleared: false, deleteStarted: false, reconciled: false }
        entries.set(key, entry)
        advanceSummaryArchiveEpoch(key)
      ...
      const runDelete = (entry, knownRevision = entry.knownRevision) => {
        ...
        if (entry.token.retryTimer !== null) return Promise.resolve(false)
    ...
    async function readSummaryManifest(signal) {
      ...
      for (const item of data.summaries) {
        ...
        if (typeof sessionId !== "string" || sessionId.length === 0 || Array.from(sessionId).length > 512) {
          return { kind: "abort" }
        }
    ...
    async function migrateCachedSummariesToHostUnlocked(signal, publishManifest, retrySessionIds, nextRetrySessionIds) {
      ...
      const sessionIds = retrySessionIds ? Array.from(retrySessionIds) : []
      if (!retrySessionIds) {
        try {
          for (let index = 0; index < localStorage.length; index += 1) {
            const key = localStorage.key(index)
      ...
      for (const sessionId of sessionIds) {
        const markRetry = () => {
          retryNeeded = true
          nextRetrySessionIds?.add(sessionId)
        }
        const archiveEpoch = summaryArchiveEpoch(sessionId)
        const blocked = () => summaryArchiveEpoch(sessionId) !== archiveEpoch || summaryDeleteTokens.has(sessionId)
        if (signal?.aborted) return
        if (blocked()) continue
        ...
        if (manifestResult.kind === "legacy") {
          ...
          if (blocked()) continue
          if (!stateRes.ok) {
            if (retryableStatus(stateRes.status)) markRetry()
            continue
          }
          try {
        ...
        try {
          ...
          if (state.summary?.value) {
            ...
            if (!dirty && baseRevision >= local.serverRevision && baseRevision > 0) {
              if (signal?.aborted) return
              if (blocked()) continue
              try { localStorage.removeItem(summaryCacheKey(sessionId)) } catch {}
              continue
          ...
          while (true) {
            ...
            if (patchRes.status !== 409) {
              if (retryableStatus(patchRes.status)) markRetry()
              break
            }
            ...
            if (currentRecord.deleted) {
              const latest = loadSummary(sessionId)
              if (latest.syncOverall || latest.syncRecent) {
                candidate = normalizedSummaryValue(latest)
                fields = { overall: true, recent: true }
                baseRevision = currentRecord.revision
                continue
              }
              if (currentRecord.revision > local.serverRevision) {
                if (signal?.aborted) return
                if (blocked()) break
                try { localStorage.removeItem(summaryCacheKey(sessionId)) } catch {}
                break
      ...
      return retryNeeded ? "retry" : undefined
    }

    async function migrateCachedSummariesToHost(signal, publishManifest, retrySessionIds, nextRetrySessionIds) {
      if (signal?.aborted) return
      if (navigator.locks?.request) {
        try {
          return await navigator.locks.request("dsh-better-ux:summary-migration", signal ? { signal } : {}, () => migrateCachedSummariesToHostUnlocked(signal, publishManifest, retrySessionIds, nextRetrySessionIds))
        } catch (err) {
          if (signal?.aborted) return
          throw err
        }
      }
      return migrateCachedSummariesToHostUnlocked(signal, publishManifest, retrySessionIds, nextRetrySessionIds)
    }
    ...
    function startCachedSummaryMigration(publishManifest) {
      ...
      const run = () => {
        ...
        running = true
        const nextRetrySessionIds = new Set()
        migrateCachedSummariesToHost(signal, publishManifest, retrySessionIds, nextRetrySessionIds).then((outcome) => {
          running = false
          if (outcome === "retry") {
            if (nextRetrySessionIds.size) retrySessionIds = nextRetrySessionIds
            scheduleRetry()
          } else {
            retrySessionIds = null
          }
        }).catch(() => {
          running = false
        })
    ...
    function ConversationSummary({ sessionId, useSession, summaryDirectory }) {
      ...
      const summarySyncRetries = React.useRef(new Map())
      ...
      React.useEffect(() => {
        ...
        return () => {
          ...
          for (const retry of summarySyncRetries.current.values()) {
            if (retry.timer !== null) window.clearTimeout(retry.timer)
          }
          summarySyncRetries.current.clear()
        }
      }, [])
      ...
      const clearSummarySyncRetry = (targetSessionId, fields = null) => {
        const key = String(targetSessionId || "")
        const retry = summarySyncRetries.current.get(key)
        if (!retry) return
        if (fields) {
          if (fields.overall) retry.fields.overall = false
          if (fields.recent) retry.fields.recent = false
          if (retry.fields.overall || retry.fields.recent) return
        }
        if (retry.timer !== null) window.clearTimeout(retry.timer)
        summarySyncRetries.current.delete(key)
      }
      const scheduleSummarySyncRetry = (targetSessionId, lifecycle, fields = { overall: true, recent: true }) => {
        const key = String(targetSessionId || "")
        const canRetry = () => Boolean(key)
          && Boolean(lifecycle)
          && activeRef.current
          && lifecycleRef.current === lifecycle
          && !lifecycle.controller.signal.aborted
          && !summaryDeleteTokens.has(key)
        if (!canRetry() || (!fields.overall && !fields.recent)) return
        let retry = summarySyncRetries.current.get(key)
        if (!retry) {
          retry = { timer: null, delay: 1000, fields: { overall: false, recent: false } }
          summarySyncRetries.current.set(key, retry)
        }
        if (fields.overall) retry.fields.overall = true
        if (fields.recent) retry.fields.recent = true
        if (retry.timer !== null) return
        const delay = retry.delay
        retry.delay = Math.min(30000, delay * 2)
        retry.timer = window.setTimeout(() => {
          ...
          const latest = loadSummary(key)
          const fields = {
            overall: retry.fields.overall && latest.syncOverall,
            recent: retry.fields.recent && latest.syncRecent,
          }
          retry.fields = { overall: false, recent: false }
          if (!fields.overall && !fields.recent) {
            summarySyncRetries.current.delete(key)
            return
          }
          patchSessionSummaryToHost(key, latest, undefined, fields, lifecycle).catch(() => {})
        }, delay)
      }
      const fetchSessionSummary = async (targetSessionId) => {
        ...
        const archiveEpoch = summaryArchiveEpoch(key)
        const canApply = () => Boolean(lifecycle)
          && activeRef.current
          && lifecycleRef.current === lifecycle
          && !lifecycle.controller.signal.aborted
          && sessionRef.current === targetSessionId
          && summaryArchiveEpoch(key) === archiveEpoch
          && !summaryDeleteTokens.has(key)
        if (!canApply()) return
        try {
          const res = await fetch(`${STATE_ROUTE}?sessionId=${encodeURIComponent(key)}`, { signal: lifecycle.controller.signal })
          if (!canApply() || !res.ok) return
          const data = await res.json().catch(() => ({}))
          if (!canApply()) return
          ...
          if (serverSummaryRecord && typeof serverSummaryRecord === "object" && serverSummaryRecord.value) {
            ...
            if (serverRevision < local.serverRevision || pendingSummaryWrites.current.has(key)) return
            ...
            if (localFields.overall || localFields.recent) {
              ...
              await patchSessionSummaryToHost(targetSessionId, { ...local, ...merged.value }, serverRevision, merged.localFields, lifecycle)
              return
            }
          ...
          } else if (!pendingSummaryWrites.current.has(key)) {
            const baseRevision = Number.isSafeInteger(data.summaryRevision) ? data.summaryRevision : 0
            const dirty = local.syncOverall || local.syncRecent
            if (baseRevision > local.serverRevision) {
              if (dirty) {
                const fields = { overall: local.syncOverall, recent: local.syncRecent }
                await patchSessionSummaryToHost(targetSessionId, local, baseRevision, fields, lifecycle)
                return
              }
              ...
              await patchSessionSummaryToHost(targetSessionId, { ...local, ...candidate }, baseRevision, fields, lifecycle)
            }
          }
        } catch {}
      }

      const patchSessionSummaryToHost = async (targetSessionId, summaryData, explicitBaseRev, changedFields = { overall: true, recent: true }, lifecycle = lifecycleRef.current) => {
        if (!targetSessionId) return "aborted"
        const key = String(targetSessionId)
        const archiveEpoch = summaryArchiveEpoch(key)
        const canWrite = () => Boolean(lifecycle)
          && activeRef.current
          && lifecycleRef.current === lifecycle
          && !lifecycle.controller.signal.aborted
          && summaryArchiveEpoch(key) === archiveEpoch
          && !summaryDeleteTokens.has(key)
        if (!canWrite()) return "aborted"
        const requestedFields = { overall: changedFields.overall === true, recent: changedFields.recent === true }
        const settleSuccessfulRetry = () => {
          clearSummarySyncRetry(key, requestedFields)
          const latest = loadSummary(key)
          const dirtyFields = {
            overall: requestedFields.overall && latest.syncOverall,
            recent: requestedFields.recent && latest.syncRecent,
          }
          if (dirtyFields.overall || dirtyFields.recent) scheduleSummarySyncRetry(key, lifecycle, dirtyFields)
        }
        ...
        try {
          ...
          while (true) {
            ...
            if (res.ok) {
              ...
              settleSuccessfulRetry()
              return "synced"
            }
            if (res.status !== 409) {
              if (retryableStatus(res.status)) {
                scheduleSummarySyncRetry(key, lifecycle, requestedFields)
                return "retry"
              }
              clearSummarySyncRetry(key, requestedFields)
              return "failed"
            ...
            const currentRecord = data.current || data.summary || data
            if (!currentRecord || !Number.isSafeInteger(currentRecord.revision)) {
              clearSummarySyncRetry(key, requestedFields)
              return "failed"
            }
            if (currentRecord.deleted) {
              ...
              if (currentRecord.revision > (summaryData.serverRevision || 0)) {
                applyTombstone(currentRecord.revision)
                clearSummarySyncRetry(key)
                return "synced"
              }
            }
            const server = normalizedSummaryValue(currentRecord.value)
          ...
          if (!canWrite() || error?.name === "AbortError") return "aborted"
          scheduleSummarySyncRetry(key, lifecycle, requestedFields)
          return "retry"
        } finally {
      ...
      const refresh = React.useCallback(async (force = false) => {
        ...
        const lifecycle = lifecycleRef.current
        const archiveEpoch = summaryArchiveEpoch(sessionId)
        const canGenerate = () => Boolean(lifecycle)
          && activeRef.current
          && lifecycleRef.current === lifecycle
          && !lifecycle.controller.signal.aborted
          && sessionRef.current === sessionId
          && summaryArchiveEpoch(sessionId) === archiveEpoch
          && !summaryDeleteTokens.has(String(sessionId))
        const resetStaleGeneration = (controller) => {
          if (requestRef.current !== controller) return
          if (!activeRef.current || lifecycleRef.current !== lifecycle || sessionRef.current !== sessionId) return
          const latest = loadSummary(sessionId)
        ...
        const run = async () => {
          ...
          try {
            ...
            if (!canGenerate()) {
              resetStaleGeneration(controller)
              return
            }
            ...
            setSummary(next)
            const syncOutcome = await patchSessionSummaryToHost(sessionId, next, undefined, activeFields, lifecycle)
            if (syncOutcome === "failed") throw new Error(tt("summary.failed"))
          } catch (error) {
            if (!canGenerate()) {
              resetStaleGeneration(controller)
              return
        ...
        if (navigator.locks?.request) {
          try {
            return await navigator.locks.request("dsh-better-ux:summary:" + String(sessionId), { signal: lifecycle.controller.signal }, run)
          } catch (error) {
            if (lifecycle.controller.signal.aborted || error?.name === "AbortError") return
            throw error
          }
        }
        return run()
    ...
    function startSettingsIcon() {
      ...
      const mark = () => {
        ...
        scheduled = false
        markHostSemantics()
        for (const button of markedSettingsCells) {
